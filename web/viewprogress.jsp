...
int mark =0;%>
<h4 style="margin-top: -5%;color: white"> <% out.println(u); %> </h4>
...
try{
    ...
    PreparedStatement ps=conn.prepareStatement("select * from score where username='"+u+"'");
...
if(lang.equalsIgnoreCase("c"))
{
    ...
    if(sc<=35){
        PreparedStatement pep = conn.prepareStatement("select * from btestval where uname = '"+u+"'");
    ...
    while(r.next()){
      ...
      for(int i=0;i<a.length;i++){
          ...
          if(a[i]!=-1){%><tr align="center"><%
             ...
             %><td><a href="adminattemptview.jsp?<%=(i+1)%>">Attempts</a></td></tr><%
             if(a[i]>2)
                 p1=p1+1;
          }
             if(a[i]==-1 && i==0){
...
<div class='barcontainer' style="margin-left:-10%">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    if(c==0 && a[0]!=-1){
        out.println("<br> Taken all the tests in the Basic level");
        %>
    
        <%
        PreparedStatement pp = conn.prepareStatement("select * from itestval where uname='"+u+"'");
      ...
      for(int i=0;i<a.length;i++){
          ...
          if(a[i]!=-1){ %><tr align="center"><%
             ...
             %> <td><%=r1s.getString(2) %></td> <%
             out.println("<td>"+a[i]+"</td>");
             %><td><a href="adminattemptview.jsp?<%=(i+7) %>">Attempts</a></td></tr><%
             if(a[i]>2)
                 p2=p2+1;
             //out.println(i);
          }
...
<div class='barcontainer' style="margin-left:-10%">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    <%
        ...
        if(c==0 && a[0]!=-1){
            out.println("<br> Taken all the tests in the Intermediate level");
           
        PreparedStatement pp1 = conn.prepareStatement("select * from atestval where uname='"+u+"'");
      ...
      for(int i=0;i<a.length;i++){
         ...
         out.println("<br> <h3 style='margin-left :130px;'>Advanced</h3>"); %>  <tr align="center"><h4><td><b>Test No</b></td> <td> <b>Marks </td></b> <td><b>View all attempts</b></td></h4></tr> 
          ...
          if(a[i]!=-1){ %><tr align="center"><%
             ...
             %> <td><%=r1s.getString(2) %></td> <%
             out.println("<td>"+a[i]+"</td>");
             %><td><a href="adminattemptview.jsp?<%=(i+13)%>">Attempts</a></td></tr><%
             if(a[i]>2)
                 p3=p3+1;
          }
             if(a[i]==-1 && i==0){
...
<div class='barcontainer' style="margin-left:-10%">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    <%
      ...
      if(c==0 && a[0]!=-1)
      {
          out.println("Taken all the tests");
      }
        ...
        out.println("Have not taken any test in basic level");
    }
    }
    }
    if(sc<=75&&sc>35){
        
        PreparedStatement pep = conn.prepareStatement("select * from itestval where uname = '"+u+"'");
    ResultSet r=pep.executeQuery();
    %> 
    <table border="3" style="margin-left:-15%"> <col width="200"><col width="200"><col width="200">
       <%
...
<div class='barcontainer' style="margin-left:-10%">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    if(c==0 && a[0]!=-1){
        ...
        out.println("<br> Taken all the tests in the Intermediate level <br>");
        %> 
        <%
        PreparedStatement pp = conn.prepareStatement("select * from atestval where uname='"+u+"'");
        ResultSet rr = pp.executeQuery();
        while(rr.next()){
        for(int i=0;i<a.length;i++){
            a[i]=rr.getInt(i+2);
 ...
 <div class='barcontainer' style="margin-left:-10%">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    <%
      ...
      if(c==0 && a[0]!=-1){
          out.println("Taken all tests");
      }
        }
    }
      if(c==0 && a[0]==-1){
          out.println("Have not taken any test in Intermediate level");
      }  
    %> <%
          PreparedStatement pp1 = conn.prepareStatement("select * from btestval where uname='"+u+"'");
        ResultSet rr1 = pp1.executeQuery();
        while(rr1.next()){
        for(int i=0;i<a.length;i++){
            a[i]=rr1.getInt(i+2);
...
<div class='barcontainer' style="margin-left:-10%">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    <%
      ...
      if(c==0 && a[0]==-1){
          out.println("Have not taken any test in advanced level");
      }
    }
       // out.println(uname);
          PreparedStatement pp1 = conn.prepareStatement("select * from btestval where uname='"+u+"'");
...
<div class="bor">
  ...
  <div class='bar' style='height:<%=a[i]*17%>%'>
    ...
    <%
          ...
          PreparedStatement pp2 = conn.prepareStatement("select * from itestval where uname='"+u+"'");
      ...
      for(int i=0;i<a.length;i++){
        ...
        out.println("<br> <h3 style='margin-left :130px;'> Intermediate </h3>");  l2=l2+1;%> <table border="3" style="margin-left:-15%"><col width="200"><col width="200"><col width="200"><tr align="center"><h4><td><b>Test No</b></td> <td> <b>Marks </td></b> <td><b>View all attempts</b></td></h4></tr> 
          ...
          if(a[i]!=-1){ %><tr align="center"><%
             ...
             %> <td><%=r1s.getString(2) %></td> <%
             out.println("<td>"+a[i]+"</td>");
 %><td><a href="adminattemptview.jsp?<%=(i+7)%>">Attempts</a></td></tr><%              p3=p3+1;
             //out.println(i);
          }
      } %></table><% n3 = p3*17;
      if(p3==6)
